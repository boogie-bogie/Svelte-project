...
const { OMDB_API_KEY, BASE_URL } = process.env;

exports.handler = async function (event, context) {
  // payload > JSON 파싱해서 params로 전환
  const params = JSON.parse(event.body);
  const { title, type, year, page, id } = params;

  const url = id
    ? `${BASE_URL}/?apikey=${OMDB_API_KEY}&i=${id}&plot=full` // 단일 영화의 상세정보
    : `${BASE_URL}/?apikey=${OMDB_API_KEY}&s=${title}&${type}&${year}&page=${page}`; //여러개 영화의 상세정보
